...
extern "C" EdlibAlignResult edlibAlign(const char* const queryOriginal, const int queryLength,
    ...
    //TODO: Figure out when to use Landau-Vishkin algorithm
    ...
    do {
        ...
        } else if (config.mode == EDLIB_MODE_HW || config.mode == EDLIB_MODE_SHW) {
...
inline int max3WithIndex(const int i1, const int i2, const int i3, int* index)
{
    if (i1 >= i2 && i1 >= i3) { *index = 1; return i1; }
    if(i2 >= i1 && i2 >= i3) { *index = 2; return i2; }
    *index = 3;
    return i3;
}
                                ...
                                int** startLocation, int** endLocation)
{
    ...
    for (int e = nk; e <= k; e++) {
        for (int d = -e; d <= e; d++) {
            if (d == -e) { //Determine which of the previous L{d,e}s is the one with the highest position in the query
