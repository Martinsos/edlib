...
struct AlignmentData {
  ...
  AlignmentData(int maxNumBlocks, int targetLength)
    : Ps(maxNumBlocks * targetLength * 2),  // enough storage for both Ps and Ms use
      Ms(Ps.data() + (maxNumBlocks*targetLength)),
      scores((maxNumBlocks + 2) * targetLength), // enough storage for scores, firstBlocks and lastBlocks
...
static inline std::unique_ptr<Word []> buildPeq(const int alphabetLength,
...
extern "C" EdlibAlignResult edlibAlign(const char* const queryOriginal, const int queryLength,
    ...
    std::vector<unsigned char> query_, target_;
    ...
    auto Peq = buildPeq(static_cast<int>(alphabet.size()), query_.data(), queryLength, equalityDefinition);
    /*-------------------------------------------------------*/
    ...
    do {
	  ...
	  /* previously, myserCalcEditDistanceSemiGlobal will assign NULL to result.endLocations
	     without checking if it holds any valid storage. As this is a loop, there is a chance
	     for memory leak. I don't know the algo well enough to make an effective test case.
	     
	     Also note that there is another malloc to result.endLocations after this do loop.
	     But it won't be used as config.mode is different.
	   */
	    myersCalcEditDistanceSemiGlobal(Peq.get(), W, maxNumBlocks,
                                            queryLength, target_.data(), targetLength,
                                            k, config.mode, result.editDistance,
                                            &(result.endLocations), &(result.numLocations));
        } else {  // mode == EDLIB_MODE_NW
	    myersCalcEditDistanceNW(Peq.get(), W, maxNumBlocks,
                                    queryLength, target_.data(), targetLength,
    ...
    //if ((result.numLocations = endLocations.size())) {
    //  result.endLocations = new int[result.numLocations];
    //  ::memcpy(result.endLocations,endLocations.data(),result.numLocations);
    //}
    ...
    if (result.editDistance >= 0) {  // If there is solution.
        ...
        if (config.task == EDLIB_TASK_LOC || config.task == EDLIB_TASK_PATH) {
	        ...
	        auto rTarget = createReverseCopy(target_.data(), targetLength);
		auto rQuery  = createReverseCopy(query_.data(), queryLength);
                // Peq for reversed query.
                auto rPeq = buildPeq(static_cast<int>(alphabet.size()), rQuery.get(), queryLength, equalityDefinition);
...
static inline std::unique_ptr<Word[]> buildPeq(const int alphabetLength,
    ...
    std::unique_ptr<Word[]> Peq(new Word[(alphabetLength + 1) * maxNumBlocks]);
...
static int myersCalcEditDistanceSemiGlobal(
        ...
        int & bestScore_, int** const positions_, int* const numPositions_) {
    //positions_.clear();
    *positions_ = nullptr;
    *numPositions_ = 0;
    ...
    std::unique_ptr<Block[]> blocks(new Block[maxNumBlocks]);
 ...
 *                         Set only if findAlignment is set to true, otherwise it is unchanged.
...
static int obtainAlignmentTraceback(const int queryLength, const int targetLength,
    ...
    // this realloc must be shrinking the array or will have unassigned values at the end
...
static int obtainAlignment(
    ...
    /* TODO: replace params alignment and alignmentLength with a std::vector output
       (*aligmnent)[(*alignmentLength)++] is like push_back, and realloc like resize
       However, need to return the alignment and length through the C API, so must
       copy from vector storage.
       Using unique_ptr makes code more rebust(exception safety) but needs to maintain
       the length separately as it does now.
     */
  
    // Handle special case when one of sequences has length of 0.
    if (queryLength == 0 || targetLength == 0) {
        auto alignmentLength = targetLength + queryLength;
	// previously this is a simple malloc call, so alignment must be nullptr. Now alignment must be empty
	// Therefore, the resize will fill alignment with only one value
        alignment.resize(alignmentLength,(unsigned char)(queryLength == 0 ? EDLIB_EDOP_DELETE : EDLIB_EDOP_INSERT));
        return EDLIB_STATUS_OK;
    }
	...
	std::unique_ptr<AlignmentData> alignData;
        auto Peq = buildPeq(alphabetLength, query, queryLength, equalityDefinition);
        myersCalcEditDistanceNW(Peq.get(), W, maxNumBlocks,
                                queryLength,
                                target, targetLength,
                                bestScore,
        ...
        statusCode = obtainAlignmentTraceback(queryLength, targetLength,
                                              bestScore, alignData.get(), alignment);
    } else {
...
static int obtainAlignmentHirschberg(
    ...
    auto Peq = buildPeq(alphabetLength, query, queryLength, equalityDefinition);
    auto rPeq = buildPeq(alphabetLength, rQuery, queryLength, equalityDefinition);
    ...
    int leftHalfCalcStatus = myersCalcEditDistanceNW(
	    Peq.get(), W, maxNumBlocks, queryLength, target, targetLength, bestScore,
    ...
    int rightHalfCalcStatus = myersCalcEditDistanceNW(
	    rPeq.get(), W, maxNumBlocks, queryLength, rTarget, targetLength, bestScore,
    ...
    std::vector<int> scoresRightOriginalStart(scoresRightLength);
    int* scoresRight = scoresRightOriginalStart.data();
    ...
    // clear vector to stay close to original delete
    scoresLeft.clear();
    scoresRightOriginalStart.clear();
...
static string transformSequences(const char* const queryOriginal, const int queryLength,
                                 const char* const targetOriginal, const int targetLength,
                                 std::vector<unsigned char> & queryTransformed_,
                                 std::vector<unsigned char> & targetTransformed_) {
    ...
    queryTransformed_.resize(queryLength);
    targetTransformed_.resize(targetLength);
    ...
    for (int i = 0; i < queryLength; i++) {
        ...
        queryTransformed_[i] = letterIdx[c];
    }
    for (int i = 0; i < targetLength; i++) {
        ...
        targetTransformed_[i] = letterIdx[c];
    }
