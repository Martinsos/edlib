 ...
 */
    ...
    EdlibAlignConfig<AlphabetType> edlibNewAlignConfig(int k, EdlibAlignMode mode, EdlibAlignTask task,
        ...
        EdlibAlignConfig<AlphabetType> config;
        config.k = k;
        config.mode = mode;
        config.task = task;
    ...
    EdlibAlignConfig<AlphabetType> edlibDefaultAlignConfig(void) {
        return edlibNewAlignConfig<AlphabetType>(-1, EDLIB_MODE_NW, EDLIB_TASK_DISTANCE, NULL, 0);
    }
        ...
        *numPositions_ = 0;
        ...
        for (int c = 0; c < targetLength; c++) { // for each column
            ...
            if ((lastBlock < maxNumBlocks - 1) && (bl->score - hout <= k) // bl is pointing to last block
                ...
                bl->M = static_cast<Word>(0);
                bl->score =
                        (bl - 1)->score - hout + WORD_SIZE + calculateBlock(bl->P, bl->M, *Peq_c, hout, bl->P, bl->M);
            } else {
 ...
 */
    ...
    static void transformSequences(const AlphabetType *const queryOriginal, const int queryLength,
        ...
        // To use the additionalEqualities, a disjoint-set
        // is instantiated to add the equivalent symbols to it.
        // After adding symbols and merging the equivalent ones,
        // each symbol is mapped to a symbol(it can be itself or another)
        // The point is that all equivalent ones will be mapped to the same symbol
        DisjointSet<AlphabetType> equalitySet;
        for (int j = 0; j < additionalEqualitiesLength; j++) {
            AlphabetType first = additionalEqualities[j].first;
            AlphabetType second = additionalEqualities[j].second;
            equalitySet.add(first);
            equalitySet.add(second);
            equalitySet.merge(first, second);
        }
        ...
        for (int i = 0; i < queryLength; i++) {
            ...
            // If the symbol is present in the disjoint-set data structure
            // find the parent symbol that represents it and replace
            if (equalitySet.find(c) != NULL) {
                c = equalitySet.find(c);
            }
