...
# except for libstdc++ (which is needed for edlib-aligner to run), in order to keep docker image small.
RUN apk --no-cache add cmake clang clang-dev make gcc g++ libc-dev linux-headers git libstdc++ && \
